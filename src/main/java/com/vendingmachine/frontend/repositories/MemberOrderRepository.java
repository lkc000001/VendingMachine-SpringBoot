...
public interface MemberOrderRepository extends JpaRepository<MemberOrder, Long>{
	...
	@Query(	value = "SELECT * " +
				"FROM MEMBERORDER " +
				"WHERE ((?1 IS NULL OR ?1 = '') OR ORDER_NO = ?1)" +
				"  AND ((?2 IS NULL OR ?2 = '') OR MEMBER_ID = ?2) " + 
	...
	@Query(	value = "SELECT * " +
			...
			"  AND ((?5 IS NULL OR ?5 = '') OR CREATE_TIME <= ?5) ",
		nativeQuery = true)
	List<MemberOrder> queryMemberOrderList(String orderNo, String memberId, Long productId, 
